...
export default function SpriteMixer(THREE) {
    ...
    function offsetTexture(actionSprite) {
        var tiles_horiz = actionSprite.mirrored
            ? -actionSprite.tilesHoriz
            : actionSprite.tilesHoriz;
        actionSprite.material.map.offset.x =
            actionSprite.getColumn() / tiles_horiz;
        actionSprite.material.map.offset.y =
            (actionSprite.tilesVert - actionSprite.getRow() - 1) /
            actionSprite.tilesVert;
    }
    ...
    function updateAction(action, milliSec) {
        ...
        while (
            ...
            action.actionSprite.currentDisplayTime -=
                action.tileDisplayDuration;
            action.actionSprite.currentTile =
                action.actionSprite.currentTile + 1;
    ...
    function getColumn() {
        if (this.mirrored) {
            return Math.abs(
                (this.currentTile % this.tilesHoriz) - (this.tilesHoriz - 1),
            );
        }
	...
	*/
    function ActionSprite(texture, tilesHoriz, tilesVert, mirror) {
        ...
        const actionSprite = new THREE.Sprite(spriteMaterial);
