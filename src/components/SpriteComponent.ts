...
export class SpriteComponent implements Component
{
    ...
    private _animations: Map<string, SpriteMixerValues> = new Map<string, SpriteMixerValues>();
    ...
    set callback(func: Function) { this._callback = func; }
        ...
        const promise = new Promise<void>((resolve, reject) => {
            new this._three.TextureLoader().load(sprite_sheet.path, (texture)=> {
                var actionSprite = this._spriteMixer.ActionSprite( 
                    texture, sprite_sheet.x_frames, sprite_sheet.y_frames, this._mirrored);
                var action = this._spriteMixer.Action(
                    actionSprite, sprite_sheet.name, 
                    0, sprite_sheet.final_frame, this._fps);
                ...
                this._animations.set(sprite_sheet.name, {actionSprite: actionSprite, action: action});
